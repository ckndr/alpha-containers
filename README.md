...
- For normal incremental workbook edits, increase the final minor number. Example: `AlphaContainers_v10_26.xlsx` becomes `AlphaContainers_v10_27.xlsx`.
- For major structural or logic changes, increase the main version. Example: `AlphaContainers_v10_26.xlsx` becomes `AlphaContainers_v11_1.xlsx`.
...
Run `Scripts\Install_Hourly_Push.bat` once on any PC where you want automatic backups. It creates a Windows scheduled task named `Alpha Containers Hourly Push` that runs `Scripts\Push.bat /auto` every hour.
